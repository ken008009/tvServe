    ...
    export default {
            ...
            return{
                columns: [
                    ...
                    },
                    {
                        title: '充值凭证',
                        dataIndex: 'image',
                        customRender: (v) => <img style="display:block;height:40px;cursor:pointer;" src={projectUrl+v} onClick={()=>this.showImage(projectUrl+v)}/>
                    },
                    {
                        title: '状态',
                        dataIndex: 'status',
        ...
        methods: {
            ...
            changeStatus(id,title,status){
                this.$confirm({
                    title: `${title}`,
                    content: `确定要${title}此充值吗?`,
                    centered: true,
                    onOk: () => {
                        return new Promise((resolve, reject) => {
                            Log[status?`recharge_adopt`:`recharge_refuse`]({ id }).then(res => {
                                resolve()
                                this.getList()
                            }).catch(res => {
                                reject()
